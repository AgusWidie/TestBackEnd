...
namespace WebSchool.Service
{
    public class DataConfigPembayaranService : IDataConfigPembayaranService
    {
        ...
        public DataConfigPembayaranService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<GlobalObjectListResponseJson<DataConfigPembayaran>> DataConfigPembayaranById(IdConfigPembayaran model, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                GlobalObjectListResponseJson<DataConfigPembayaran> resUnauthorization = JsonSerializer.Deserialize<GlobalObjectListResponseJson<DataConfigPembayaran>>(jsonString, options);
                return resUnauthorization;
            }

            var httpWebRequest = (HttpWebRequest)WebRequest.Create(_apiConfig.BaseUrl + _apiConfig.UriDataConfigPembayaranById);
            ...
            GlobalObjectListResponseJson<DataConfigPembayaran> resData = JsonSerializer.Deserialize<GlobalObjectListResponseJson<DataConfigPembayaran>>(content, options);
            return resData;
        }
        ...
        public async Task<GlobalObjectListResponseJson<DataConfigPembayaran>> DataConfigPembayaranByNama(NamaConfigPembayaran model, CancellationToken cancellationToken)
        {
            ...
            var httpWebRequest = (HttpWebRequest)WebRequest.Create(_apiConfig.BaseUrl + _apiConfig.UriDataConfigPembayaranByNama);
            httpWebRequest.Headers.Add("Authorization", "Bearer " + Token + "");
            httpWebRequest.Headers.Add("Strict-Transport-Security", "max-age=31536000");
            httpWebRequest.Headers.Add("X-Content-Type-Options", "nosniff");
            httpWebRequest.Headers.Add("X-Xss-Protection", "1; mode=block");
