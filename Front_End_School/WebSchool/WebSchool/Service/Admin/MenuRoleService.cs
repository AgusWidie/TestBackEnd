...
namespace WebSchool.Service
{
    public class MenuRoleService : IMenuRoleService
    {
        ...
        public MenuRoleService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            _apiConfig = configuration.GetSection(nameof(ApiConfiguration)).Get<ApiConfiguration>();
            _httpContextAccessor = httpContextAccessor;
        }

        public async Task<GlobalObjectListResponseJson<MenuRole>> CheckAddMenuRole(CheckMenuRoleModel model, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                GlobalObjectListResponseJson<MenuRole> resUnauthorization = JsonSerializer.Deserialize<GlobalObjectListResponseJson<MenuRole>>(jsonString, options);
                return resUnauthorization;
            }

            var httpWebRequest = (HttpWebRequest)WebRequest.Create(_apiConfig.BaseUrl + _apiConfig.UriCheckAddMenuRole);
            ...
            GlobalObjectListResponseJson<MenuRole> resData = JsonSerializer.Deserialize<GlobalObjectListResponseJson<MenuRole>>(content, options);
            return resData;
        }

        public async Task<GlobalObjectListResponseJson<MenuRole>> AddMenuRole(MenuRoleInputModel model, CancellationToken cancellationToken)
        {
            GlobalResponseJson res = new GlobalResponseJson();
            ...
            var httpWebRequest = (HttpWebRequest)WebRequest.Create(_apiConfig.BaseUrl + _apiConfig.UriAddMenuRole);
            httpWebRequest.Headers.Add("Authorization", "Bearer " + Token + "");
            httpWebRequest.Headers.Add("Strict-Transport-Security", "max-age=31536000");
            httpWebRequest.Headers.Add("X-Content-Type-Options", "nosniff");
            httpWebRequest.Headers.Add("X-Xss-Protection", "1; mode=block");
